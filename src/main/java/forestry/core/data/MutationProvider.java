...
public class MutationProvider implements DataProvider {
	...
	private void bees() {
		...
		// Structured: every pairwise cross of the 5 rocky wilds (mirrors Common)
		...
		// Mineral: Structured x any one wild (mirrors Cultivated)
		for (ResourceLocation wild : rockyWilds) {
			add(ForestrySpeciesTypes.BEE, STRUCTURED, wild, MINERAL, 0.10f).cave();
		}

		// First split: same pair, two alternate results (mirrors Common x Cultivated -> Diligent/Noble)
